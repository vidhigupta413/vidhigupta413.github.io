...
import { coffeeChat, contact, aboutMe } from '../../../data/portfolio.js';

const links = [
  {
    label: 'Email',
    value: contact.email,
    href: `mailto:${contact.email}?subject=Coffee%20Chat`,
    icon: (
...
export default function CoffeeChatPanel() {
  return (
    <OverlayShell title={coffeeChat.title} subtitle={coffeeChat.subtitle} accent="Far corner · Persona">
      <p className="mb-5 text-sm leading-relaxed text-white/80">{coffeeChat.intro}</p>
      <ul className="mb-6 list-disc space-y-2 pl-5 text-sm text-white/75">
        {coffeeChat.bullets.map((b) => (
          <li key={b}>{b}</li>
        ))}
      </ul>
      <p className="mb-4 text-xs uppercase tracking-widest text-cafe-neon-purple">{coffeeChat.cta}</p>
